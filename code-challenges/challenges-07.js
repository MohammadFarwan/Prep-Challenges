...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            let fullName = "";
            let fName = arr[i].firstName;
            let lName = arr[i].lastName;
            if (lName == null) {
                fullName = fName;
            } else {
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        for (let j = 0; j < data.grades[i].classes.length; j++) {
            let sum = 0;
            for (let c = 0; c < data.grades[i].classes[j].classScores.length; c++) {
                sum += data.grades[i].classes[j].classScores[c];
            }
            data.grades[i].classes[j].avg = Math.round(sum / data.grades[i].classes.length)
        }
    }
};
// -------------------------------------------------------------------------------------------------------
