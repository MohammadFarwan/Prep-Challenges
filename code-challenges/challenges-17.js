...
const recursionPattern = (int1, int2) => {
    ...
    const generatePattern = (current, decreasing) => {
        pattern.push(current);
        if (decreasing) {
            if (current - int2 > 0) {
                generatePattern(current - int2, true);
            } else {
                generatePattern(current - int2, false);
            }
        } else {
            if (current + int2 <= int1) {
                generatePattern(current + int2, false);
            }
        }
    };

    generatePattern(int1, true);
    return pattern;
};
// -------------------------------------------------------------------------------------------------------
...
const filterLinks = (str) => {
    const regex = /<a\s+href="https?:\/\/(www\.[^"]+\.(com|org|net))"/;
