...
const employeesBonus = (arr) => {
    // write your code here
    arr.forEach(function(obj) {(obj.workHours > 8 ) ? obj.salary = `${+obj.salary.slice(0, -1) + 100}$` : obj.salary = `${+obj.salary.slice(0, -1) + 50}$`;}); return arr;
}
// -------------------------------------------------------------------------------------------------------
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    // write your code here
    let bud = budget.slice(0, -1);
    let max = 0;
    mouseArray.forEach((element) => {
        for (let k of keyBoardArray) {
            if ((element + k) > max && (element + k) <= bud) {
                max = (element + k);
            }
        }
    }
